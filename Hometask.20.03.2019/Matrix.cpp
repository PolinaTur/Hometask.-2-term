...
int* ValidationOfArray(int* array, int n)
{
	try
	{
		n = ValidationOfElement(n);
...
int** ValidationOfMatrix(int** matrix, int* array, int n)
{
	if (matrix == nullptr)
	{
...
void RandomMatrix(int** matrix, int n, int* dimentions)
{
	for (int i = 0; i < n; i++)
	{
...
void QuickSort(int* array, int n, Key key)
{
...
void GetSorted(int** matrix, int n, int* dimentions, Sort sort, Key key)
{
	for (int i = 0; i < n; i++)
	{
		if (dimentions[i] % 2)
